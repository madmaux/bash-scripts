...
if [ "$already" != "y" ]; then
  ...
  linux_headers=`curl -s -L --fail --metalink "$kernelURL"|grep -o '<a .*href="linux-headers.*">' | sed -e 's/<a .*href=['"'"'"]//' -e 's/["'"'"'].*$//' -e '/^$/ d'|grep generic|grep amd64.deb|sed -e '1d'`

  linux_image=`curl -s -L --fail --metalink "$kernelURL"|grep -o '<a .*href="linux-image.*">' | sed -e 's/<a .*href=['"'"'"]//' -e 's/["'"'"'].*$//' -e '/^$/ d'|grep generic|grep amd64.deb|sed -e '1d'`

  linux_deb=`curl -s -L --fail --metalink "$kernelURL"|grep -o '<a .*href="linux-headers.*">' | sed -e 's/<a .*href=['"'"'"]//' -e 's/["'"'"'].*$//' -e '/^$/ d'|grep all.deb|sed '1,5d'`
